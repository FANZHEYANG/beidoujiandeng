...
static void BQ25601_writeRegister(uint8_t addr, uint8_t value)
{
    ...
    I2C_Send7bitAddress(BQ25601_SLAVE_ADDR, I2C_Direction_Transmitter);
...
static uint8_t BQ25601_readRegister(uint8_t addr)
{
   ...
   //I2C_GenerateSTOP(ENABLE);
    ...
    I2C_Send7bitAddress(BQ25601_SLAVE_ADDR, I2C_Direction_Receiver);//发送地址+最低位1表示为“读”
...
{
	uint8_t temp_reg = 0;
	//  0x05:set input current limit to 500 mA
	//  0x0A:set input current limit to 1000 mA
	temp_reg = BQ25601_readRegister(REG00);
	if((temp_reg & 0x1F) != 0x0A)
	{
		temp_reg &= 0xE0;
		temp_reg |= 0x0A;
	...
	// 0x06:set charge current 360 mA
	// 0x0A:set charge current 600 mA
	temp_reg = BQ25601_readRegister(REG02);
	if((temp_reg & 0x3F) != 0x0A)
	{
		temp_reg &= 0xC0;
		temp_reg |= 0x0A;
...
// 	uint8_t out_report_data[3] = {COMMANDS_ID, READ_BQ25601_FAULTS, 0};
...
// 		{
// 			commands_queue->Insert(out_report_data);
// 		}
// 	}
// }
